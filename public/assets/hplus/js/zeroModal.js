...
(function(obj) {
    ...
    function _buildModal(opt) {
        ...
        //// 构建容器
        var _container = $('<div zero-unique-container="' + opt.unique + '" class="zeromodal-container" style="z-index:' + _tmp_last_zindex + ';width:' + _width + 'px;height:' + _height + 'px;left:' + _left + 'px;top:' + (opt.transition ? _top - 50 : _top) + 'px;border:3px solid #E95413;border-bottom:1px solid #E95413;"></div>');
        $('body').append(_container);

        //// 判断是否需要允许拖拽显示层
        if (opt.drag) {
            var _dragTopper = $('<div zero-unique-top="' + opt.unique + '" class="zeromodal-top"><div title="关闭" zero-close-unique="' + opt.unique + '" class="zeromodal-close">×</div></div>');
        ...
        _headerHtml += '        <div class="modal-title"  style="font-weight: 400;    margin-top:15px;    font-size:18px;text-align: center;"   >' + opt.title + '</div>';
        _headerHtml += '   </div>';
